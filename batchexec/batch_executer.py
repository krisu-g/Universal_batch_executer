...
class UniversalBatchExecuter:
    ...
    def __init__(self, config_file_name='config.json'):
        ...
        if not config_file_name.is_file():
            raise IOError(f'the config file: {config_file_name} does NOT exist. Create it based on documentation and run again.')
        else:
    ...
    def process_data(self, data):
        ...
        if not isinstance(data['file_masks'], list) and all(isinstance(file_mask, str) for file_mask in data['file_masks']):
        ...
        data['app_name'] = (data['app_dir'] / Path(data.get('app_name', 'app.exe'))).resolve()
        ...
        if not isinstance(data['app_input_parameter'], str):
            error_msg += f'Application input parameter (app_input_parameter): "{data["app_input_parameter"]}" should be a string.\n'
        data['app_output_parameter'] = data.get('app_output_parameter', '-o')
        if not isinstance(data['app_output_parameter'], str):
            error_msg += f'Application output parameter (app_output_parameter): "{data["app_output_parameter"]}" should be a string.\n'
        data['app_output_file_extension'] = data.get('app_output_file_extension', '')
        if not isinstance(data['app_output_file_extension'], str):
            error_msg += f'Output file extension (app_output_file_extension): "{data["app_output_file_extension"]}" should be a string.\n'
    ...
    def run(self, additional_args=''):
        ...
        file_stdout = open(self.output_to, 'a+', encoding='utf8') # todo: use option is_append or sth
        file_stdout.write(80 * '=' + '\n')
        file_stdout.write((datetime.datetime.now()).strftime('%d/%m/%Y %H:%M:%S') + '\n')
        file_stderr = open(self.error_output_to, 'a+', encoding='utf8') # todo: use option is_append or sth
        ...
        for f in files:
            ...
            if self.app_output_file_extension:
                output_file_path = self.output_dir.joinpath(relative_file_path).with_suffix(self.app_output_file_extension)
            else:
...
if __name__ == 'main':
    ...
    parser = argparse.ArgumentParser(
        description='Execute batch command with options from configuration file.'
    )
    parser.add_argument('config_file', default='config.json', help='file with config parameters to batch execute command.')
